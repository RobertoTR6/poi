...
const menuStructure = {
    configuracion: { name: 'Configuración' }, planeamiento: { name: 'Planeamiento' },
    cmn: { name: 'CMN' }, evaluacion: { name: 'Evaluación' },
};

export function init() {
    setupCrudController({
        collection: 'users', getId: item => item.id,
        tableBodyId: 'tableUsuario', searchInputId: 'searchUsuario', createBtnId: 'btnNuevoUsuario',
        downloadBtnId: 'downloadUsuario', importBtnId: 'btnImportarUsuario', fileImporterId: 'file-importer-usuario',
        modalId: 'modal-usuario', formId: 'form-usuario', modalTitleId: 'modalUsuarioTitle',
        ...
        readForm: (form, item) => { const permisos = {}; Object.keys(menuStructure).forEach(modId => { const check = form.querySelector(`#check-${modId}`); permisos[modId] = check?.checked ? (form.querySelector(`input[name="perm-${modId}"]:checked`)?.value || 'visualizar') : null; }); const clave = form.elements.clave.value; return { id: item?.id, nombre: form.elements.nombre.value.trim(), usuario: form.elements.usuario.value.trim(), clave: clave ? clave : item?.clave, centroCostoCodigo: form.elements.centroCostoCodigo.value, permisos }; },
        csvConfig: { filename: 'usuarios_plantilla.xlsx', headers: 'Nombre,Usuario,Clave,CodigoCentroCosto', formatRow: (item) => [item.nombre, item.usuario, '', item.centroCostoCodigo] },
        importConfig: { uniqueKey: 'usuario', headers: { 'Nombre': 'nombre', 'Usuario': 'usuario', 'Clave': 'clave', 'CodigoCentroCosto': 'centroCostoCodigo' } }
    });
}